// Package main shows how to back the client's HTTP cache with Redis, so cached
// responses are shared across every process that points at the same server.
//
...
func main() {
	...
	listener, err := listenConfig.Listen(ctx, "tcp", "127.0.0.1:18080")
	if err != nil {
		log.Fatalf("port 18080 is busy: %v", err)
	}
	...
	// Host and Port default to localhost:6379, which is what docker-compose.yml
	// publishes. Password and DB are available for a real deployment.
	cache := httpclient.NewRedisClientCache(httpclient.RedisConfig{})
	defer func() {
		if closeErr := cache.Close(); closeErr != nil {
			log.Printf("closing redis: %v", closeErr)
		}
	}()
	...
	if pingErr := cache.Ping(ctx); pingErr != nil {
		log.Fatalf("redis is not reachable — did you run `docker compose up -d redis`? %v", pingErr)
	}
	...
	fmt.Println("\nEntries live in Redis, so a second run of this example starts already warm.")
}
